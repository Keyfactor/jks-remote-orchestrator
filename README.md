...
**PLEASE NOTE:** This integration has been decprecated and replaced by The RemoteFile Orchestrator Extension - https://github.com/Keyfactor/remote-file-orchestrator. <br><br> The remote JKS Orchestrator Extension allows for the remote management of Java Key Stores. Discovery, Inventory, and Management functions are supported. The orchestrator extension performs operations by issuing remote commands over SSH to Linux based systems and WinRM to Windows based systems.
...
- **Supported Job Types, General Settings, Password Settings, Store Path Types, and Other Settings** – All checked/unchecked as shown.  For "Supports Entry Password", this is optional.  Selecting this allows users to specify a separate password from the certificate store password to be used as the key password for that entry when enrolling a new certificate with certificate store delivery or adding an existing certificate to a store.  If this selection is checked for the certificate store type definition but the entryPassword field is left blank when adding a certificate to a store, the store password will be used for the key password.
